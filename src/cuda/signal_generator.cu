...
__device__ double sim_microphone(double x, double y, double z, double* angle, char mtype) {
...
__device__ void copy_previous_rir(double* imp, int row_idx, int nsamples) {
    if (row_idx == 0) {
...
__device__ void hpf_imp(double* imp, int row_idx, int nsamples, hpf_t hpf) {
...
__device__ void mexFunction(const long signal_length, const double *in, const double fs, const double *rr, const double *ss, double *out) {
    // Set parameters
    double          c = 340.0;
    double          LL[3] = {10, 10, 3};
...
//__constant__ double ROOM_DEPTH = 9.0;
//__constant__ double ROOM_WIDTH = 9.0;
//__constant__ double ROOM_HEIGHT = 3.0;

// Kernel funciton 
__global__ void kernel(const long signal_length, const double *in, const double fs, double *out) {
